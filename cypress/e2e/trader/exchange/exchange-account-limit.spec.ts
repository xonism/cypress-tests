...
describe('Trader - Exchange - Tier Limits', () => {
  ...
  it('Check if unverified user can create exchange order', () => {
    ...
    cy.assertReceiveAmountInInputAndOrderDetailsMatch($.TRADER.EXCHANGE.INPUT.RECEIVE_AMOUNT, receiveCurrency.currencySymbol)
  ...
  it('Check if user can create exchange order above the monthly limit', () => {
    ...
    cy.get($.TRADER.MOBILE.DIV.TRADER_LIMITS)
      ...
      .then((swapLimitText) => {
        ...
        cy.selectCryptoCurrency($.TRADER.BUY.DROPDOWN.RECEIVE_CURRENCY, sellCurrency)
        ...
        cy.get($.TRADER.BUY.INPUT.RECEIVE_AMOUNT)
          ...
          .then((limitValue: string) => {
            ...
            cy.get($.TRADER.EXCHANGE.INPUT.SELL_AMOUNT)
              .typeAndAssertValueWithPulseCheck(limitValue)
  ...
  it('Check if user can create exchange order after passing monthly limit', () => {
    cy.simplyVerifyUserViaAPI(traderEmail)
    ...
    cy.get($.TRADER.MOBILE.DIV.TRADER_LIMITS)
      ...
      .then((swapLimitText) => {
        ...
        cy.get($.TRADER.BUY.INPUT.PAY_AMOUNT)
          .typeAndAssertValueWithPulseCheck(swapLimit)
