...
describe('Mobile - Billing - Billing Subscriptions - Quarterly', () => {
  ...
  it('[Mobile] Create new quarterly subscription', () => {
    ...
    cy.get($.GENERAL.COLLAPSE.HEADER) // TODO: add selector
      .should('contain', billingPaymentMethod)
      .and('contain', `${Number(billingItem.itemPrice)} ${fiatCurrencyCode}`)

    cy.get($.GENERAL.COLLAPSE.CONTENT) // TODO: add selector
      .should('be.visible')
    ...
    cy.contains($.GENERAL.DESCRIPTIONS.DESCRIPTIONS, 'Details') // TODO: add selector
      .should('be.visible')
      .within(() => {
