...
describe('Merchant - API - Callbacks', () => {
  ...
  it('Check if request with "Pending" order status appears in "Callbacks"', () => {
  ...
  it('Check if request with "Paid" order status appears in "Callbacks"', () => {
    cy.createApiAppViaAPI()
      .then((response) => {
        const AUTH_TOKEN = response.body.api_app.auth_token
